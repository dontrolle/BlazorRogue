...
`Dockerfile.graphics`/`Dockerfile.graphics.dockerignore` are a **local-build-only** variant that
bundles whatever tileset is present on disk, for producing a hosted playtest build. Never build it
in CI (the tileset isn't there) and never push the resulting image to a public registry — it embeds
...
- Never let `wwwroot/img/uf_*` (the proprietary tileset) end up in a publicly-shipped or
  publicly-pushed artifact — see the `Dockerfile.graphics` note above.
