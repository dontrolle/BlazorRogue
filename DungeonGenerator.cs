...
class DungeonGenerator
{
    ...
    // Decorations
    const double PercentageChanceOfBones = 0.05;
    readonly double percentageChanceOfTables = 0.06;
    readonly double percentageChanceOfAltars = 0.04;
    readonly double percentageChanceOfSpiderWebInCorner = 0.25;
    readonly double percentageChanceOfTorch = 0.25;
    readonly double percentageChanceOfChests = 0.02;

    // TODO: UF
    readonly string[] doorTypes = ["metal", "stone", "wood", "ruin"];
    ...
    Tuple<int, int> GetRandomUnblockedMapTile()
    {
        int maxSearch = 200;
    ...
    void AddPostGenerationDecorations()
    {
        for (int x = 0; x < map.Width; x++)
        {
            for (int y = 0; y < map.Height; y++)
    ...
    void AddPostGenFloorDecorations(int x, int y)
    {
        if (map.Tiles[x, y].TileType == TileType.Floor)
        {
            if (
    ...
    int NumberOfSurroundingBlockingSpots(int x, int y)
    {
    ...
    bool MapTileContainsDoor(int x, int y) => map.GameObjectByCoord[x, y].Any(go => go is Door);
...
#pragma warning restore CA1851 // Possible multiple enumerations of 'IEnumerable' collection
    ...
    bool GetRandomBool() => random.Next(0, 2) == 0;
    ...
    void PlaceFloor(int x, int y, TileSet floorSet)
    {
