...
/// </param>
sealed class AdamMilVisibility(
    Func<int, int, bool> blocksLight,
    Action<int, int> setVisible,
    Func<int, int, int> getDistanceSquared
) : Visibility
{
    public override void Compute(LevelPoint origin, int rangeLimit)
    {
        setVisible((int)origin.X, (int)origin.Y); // cast to int: we live on the wild side and aren't afraid of overflow
        for (uint octant = 0; octant < 8; octant++)
            Compute(octant, origin, rangeLimit, 1, new Slope(1, 1), new Slope(0, 1));
    }

    readonly struct Slope(uint y, uint x) // represents the slope Y/X as a rational number
    {
            ...
            else // top < 1
            {
                ...
                if (BlocksLight(x, topY, octant, origin)) // if the tile blocks light (i.e. is a wall)...
                {
                    ...
                    if (
                        ...
                        && !BlocksLight(x, topY + 1, octant, origin)
                    )
                    {
                        topY++;
                    }
    ...
    void SetVisible(uint x, uint y, uint octant, LevelPoint origin)
    {
        ...
        setVisible((int)nx, (int)ny);
    }

    readonly Func<int, int, bool> blocksLight = blocksLight;
    readonly Func<int, int, int> getDistanceSquared = getDistanceSquared;
    readonly Action<int, int> setVisible = setVisible;
}
